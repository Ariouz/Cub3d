...
# define MULTIPLE_FILE_TO_SAME_TEXTURE "Multiple file to the same texture\n"
# define MULTIPLE_COLOR_TO_SAME_TEXTURE "Multiple color to the same texture\n"
...
# define MAP_DESCRIPTION_PLAYER_ERROR "The map must have only one player\n"
...
void			draw_walls(t_game *game, t_raycast *ray,
					t_player *player, t_vector infos);
void			draw_floor_ceil(t_vector vars, t_vector infos,
					t_vector line_vec, t_game *game);
...
void			put_pixel_img_radius(t_img img, t_vector vec,
					int color, int thickness);
void			put_img_to_img(t_img *dst, t_img *src, int x, int y);
void			draw_line_to_img(t_img *img, t_vector v1,
					t_vector v2, int color);
void			draw_rect_to_img(t_img *img, t_vector v1,
					t_vector v2, int color);
