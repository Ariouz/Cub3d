...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/05/23 07:26:05 by gurousta          #+#    #+#             */
/*   Updated: 2024/05/23 09:23:11 by gurousta         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_east_asset(t_game *game, t_assets *assets, char *line)
{
	...
	printf("path to the east texture: %s\n", temp); // TO DELETE ONLY FOR TESTING
	assets->east_wall = mlx_xpm_file_to_image(game->mlx, temp, &w, &h);
	free(temp);
	if (assets->east_wall == NULL)
